...
sns.set(style='whitegrid', context='paper', palette='colorblind', font_scale=1.8)
...
for data, category in zip(args.datafiles, args.modelfiles):
    for prob in args.probs:
        ...
        for run in range(args.n_runs):
            hdn = pd.read_json(f"hidden/val_hidden_{m_name}_{run}.json",encoding='utf-8')
            words_dataset = pd.concat([words_dataset, hdn], axis=0, ignore_index=True)
        ...
        words_dataset = pd.DataFrame()
        for run in range(args.n_runs):
            hdn = pd.read_json(f"hidden/val_hidden_{m_name}_{run}_trained.json",encoding='utf-8')
            words_dataset = pd.concat([words_dataset, hdn], axis=0, ignore_index=True)
            
            hdn = pd.read_json(f"hidden/test_hidden_{m_name}_{run}_trained.json",encoding='utf-8')
            words_dataset = pd.concat([words_dataset, hdn], axis=0, ignore_index=True)
        del hdn
            ...
            for word, lab in zip(eval_words.data, eval_words.label):
                ...
                for i, (f_v, t_v) in vectorizer.vectorize_single_char(word):
                    ...
                    _, out_rnn, _ = model(f_v.unsqueeze(0), 
                                            ...
                                            hidden, 
                                            args.drop_p)
                    rep += torch.flatten(out_rnn.squeeze(0)[-1].detach()).numpy()
        ...
        df = pd.concat([df, pd.DataFrame(hidd_repr)], axis=0, ignore_index=True)
        ...
        df_trained = pd.concat([df_trained, pd.DataFrame(hidd_repr)], axis=0, ignore_index=True)
        
        mappings = {'ES':'ES', 'EN':'L2', 'EU':'L2', 'ES+':'ES+', 'ES-':'ES-'}
        ...
        for data_label, data_df in zip(datas, dfs):
            for run in range(args.n_runs):
                ...
                for tp, D in zip(types, mats):
                    if grp == 'MONO' and 'L2' in tp: continue
                    similarities['dataset'].append(category)
                    similarities['prob'].append(end)
                    similarities['Version'].append(grp)
                    similarities['run'].append(run)
                ...
                if plot:
                    if run == 0 and category == 'ESEN' and prob == 60:
                        ...
                        b = tmp[(tmp.Language == 'ES') | (tmp.Language == 'L2')]
                        ...
                        a = tmp[(tmp.Language == 'ES+') | (tmp.Language == 'ES-')]
